...
function getDataDir(): string {
  ...
  try {
    ...
    // Probe writeability (some hosts have read-only CWD)
...
/** Append a cart quote order without creating or updating a customer portal account. */
export function appendDynamicQuoteOrder(order: OrderRecord): void {
  const ordersData = readDynamicOrders();
  ordersData.orders.push(order);
  writeDynamicOrders(ordersData);
}
