...
type FoldSide = {
  depth: number; // inches
  angle: number; // degrees (0 = flat, 90 = bent, 180 = folded back fully)
};

export type AcmPanelPreview3DProps = {
  width: number; // overall width in inches
  length: number; // overall length in inches
  thickness?: number; // mm, visual only
  /** Fill color when no `swatchImage` (hex). With a texture, multiplies the map — use white for true swatch match. */
  color?: string;
  /** Public URL to the catalog swatch image (e.g. `color.swatchImage`). Same asset as the grid; center and returns use one material. */
  swatchImage?: string;
  topFold?: FoldSide;
  bottomFold?: FoldSide;
  leftFold?: FoldSide;
...
type FoldPanelProps = Omit<AcmPanelPreview3DProps, "swatchImage"> & {
  /** Loaded texture, or null for solid `color` only */
...
function getSceneScale(width: number, length: number) {
  const largest = Math.max(width, length, 1);
  return 3.5 / largest;
}
...
export function calculateBendAllowance(
  ...
  return angleRad * (insideRadius + kFactor * materialThickness);
}
...
export function calculateSetback(
  ...
  const angleRad = degToRad(angleDeg);
  if (Math.tan(angleRad / 2) === 0) return 0;
  return (insideRadius + materialThickness) * Math.tan(angleRad / 2);
}
...
export function calculateBendDeduction(
  ...
  const setback = calculateSetback(angleDeg, insideRadius, materialThickness);
  const ba = calculateBendAllowance(angleDeg, insideRadius, materialThickness, kFactor);
  return 2 * setback - ba;
}

/** Loads `imageUrl` and renders the same panel mesh tree with a textured material. */
