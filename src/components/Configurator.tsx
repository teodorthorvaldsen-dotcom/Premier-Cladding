...
import { sizeSelectionToFoldPreview } from "@/lib/acmFoldPreview";

const AcmPanelPreview3D = dynamic(() => import("./AcmPanelPreview3D"), {
  ssr: false,
  loading: () => (
    <div
      className="flex h-[360px] items-center justify-center rounded-xl bg-gray-50 text-sm text-gray-500"
      aria-hidden
    >
      Loading 3D preview…
    </div>
...
export function Configurator() {
  ...
  const thicknessMm = Number(thicknessId.replace("mm", ""));
  const foldPreview = sizeSelectionToFoldPreview(size);
  ...
  const handleAddToCart = () => {
    ...
    addItem({
      ...
      ...(size.leftReturnIn != null && size.leftReturnIn > 0
        ? { leftReturnIn: size.leftReturnIn }
        : {}),
      ...(size.rightReturnIn != null && size.rightReturnIn > 0
        ? { rightReturnIn: size.rightReturnIn }
        : {}),
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-12 md:gap-8 lg:gap-16">
        ...
        >
          ...
          >
            ...
            >
              <h2
                id="panel-preview-heading"
                className="text-[15px] font-medium uppercase tracking-wider text-gray-500"
              >
                Panel preview
              </h2>
              <p className="mt-1 text-[11px] text-gray-500">
                Drag to rotate. Optional returns show L- or U-shaped channels (90° in this model).
              </p>
              <div className="mt-3">
                <AcmPanelPreview3D
