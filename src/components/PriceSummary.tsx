...
export function PriceSummary({ pricing, loading = false, error = null }: PriceSummaryProps) {
  ...
  return (
    <div className={cardBase}>
      <div className={bodyBase}>
        {loading && (
          <p className="mb-4 text-[13px] text-gray-500" aria-live="polite">Updating price…</p>
        )}
        <div className="mb-6">
          <p className="text-[28px] font-semibold tabular-nums tracking-tight text-gray-900">
            {formatUSD(pricing.pricePerSqFt)} <span className="text-base font-medium text-gray-600">/ sq ft</span>
          </p>
        </div>
        <dl className="space-y-3.5 text-[15px]">
          <div className="flex justify-between">
            <dt className="text-gray-600">Panel type</dt>
            <dd className="tabular-nums text-gray-900">{pricing.panelTypeLabel}</dd>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between border-t border-gray-200 pt-4">
            ...
            <dd className="text-xl font-bold tabular-nums text-gray-900">{formatUSD(pricing.total)}</dd>
          </div>
        </dl>
